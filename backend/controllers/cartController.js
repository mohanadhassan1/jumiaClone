...
const addToCart = async (req, res) => {
  try {
    ...
    if (existingCart) {
      ...
      return res.status(200).json({ message: 'Product added to cart successfully', cart: existingCart });
    } else {
      // If the cart doesn't exist, create a new cart
      const newCart = await CartModel.create({ cart_id, customer_id, items: [{ product_id, quantity }] });
      return res.status(201).json({ message: 'Cart created and product added successfully', cart: newCart });
    }
  } catch (error) {
...
cart.items[itemIndex].quantity = new_quantity
    ...
    await cart.save();
    return res.status(200).json({ message: 'Cart item updated successfully', cart: cart });
  } catch (error) {
...
const deleteCartItem = async (req, res) => {
  try {
    ...
    console.log(index)
      cart.items.splice(index,2)
      await cart.save();

    return res.status(200).json({ message: 'Cart item deleted successfully', cart: cart });
  } catch (error) {
